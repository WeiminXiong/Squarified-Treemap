{
  ...
  function treemap(data, width, height) {
    ...
    // 计算叶节点位置，判断是否转变方向
    // 保留第一层信息，方便染色
    function calcPos(childlist, rowlist, x, y, width, height, parent, score) {
      ...
      // 将已经得到节点安置
      function layout(nodelist, value, x, y, direction, height, width, parent) {
        //比例尺
        let scale;
        //横向
        if (direction == 1)
      ...
      if (childlist.length == 0) {
        ...
        layout(rowlist, total, x, y, direction, height, width, parent);
      }
      // 对当前层的剩余节点进行计算
      else {
        let c = childlist[0];
        ...
        if (score >= new_score) {
          ...
          calcPos(childlist, newlist, x, y, width, height, parent, new_score);
        }
        else
        // 更换位置重新安放
        {
          ...
          layout(rowlist, total, x, y, direction, height1, width1, parent);
          if (direction == 1)
            calcPos(childlist, [], x, y + height1, width, height - height1, parent, 100000000);
          else
            calcPos(childlist, [], x + width1, y, width - width1, height, parent, 100000000);
        }
      }
    }
